...
(function () {
  ...
  function drawSecondaryBall() {
    ctx.save();
    ctx.beginPath();
    ctx.arc(secondaryBall.x, secondaryBall.y, secondaryBall.radius, 0, Math.PI * 2);
  ...
  function drawPlayer() {
    ...
    ctx.fillStyle = '#ffffff';
    ctx.font = '700 12px -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('PLAYER', player.x + player.width / 2, player.y + player.height / 2);
    ctx.restore();
  }
  ...
  function updatePlayer() {
    ...
    if (activeKeys['ArrowRight'] || activeKeys['KeyD'] || activeKeys['d'] || activeKeys['D']) {
      player.x += player.speed;
    }
    if (activeKeys['ArrowUp'] || activeKeys['KeyW'] || activeKeys['w'] || activeKeys['W']) {
      player.y -= player.speed;
    }
    if (activeKeys['ArrowDown'] || activeKeys['KeyS'] || activeKeys['s'] || activeKeys['S']) {
      player.y += player.speed;
    }
